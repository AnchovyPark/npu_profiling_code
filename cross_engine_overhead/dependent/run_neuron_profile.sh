#!/bin/bash
# Cross-Engine Overhead Neuron Profiling Script
#
# 각 cross-engine 조합별 Python 스크립트 실행 → NEFF 생성 → neuron-profile capture → JSON 추출
#
# Usage:
...
# Section 1: matmul 시작 (TensorEngine →)
# Section 2: norm/softmax 시작 (→ TensorEngine)
# Section 3: add 시작 (residual → norm)
# Section 4: silu 시작 (activation → gate)
...
echo -e "${GREEN}========================================${NC}"
echo -e "${GREEN}Cross-Engine Overhead Neuron Profiling${NC}"
...
for PAIR in $PAIRS; do
    ...
    # Step 0: 이전 NEFF 정리
    echo -e "${YELLOW}[0] 기존 NEFF 정리 (neuron_cross_${PAIR}_*)${NC}"
    rm -rf /tmp/neuron_cross_${PAIR}_*
    ...
    # Step 2: NEFF 디렉토리 찾기
    NEFF_DIRS=$(find /tmp -maxdepth 1 -type d -name "neuron_cross_${PAIR}_*" 2>/dev/null | sort)
    ...
    for COMPILER_DIR in $NEFF_DIRS; do
        ...
        SHAPE=$(echo "$DIR_NAME" | sed "s/neuron_cross_${PAIR}_//")
