#!/bin/bash
# matmul_diverse dependent profiling
...
for SCRIPT in $SCRIPTS; do
    ...
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BLUE}  ${NAME} (dependent)${NC}"
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

    echo -e "${YELLOW}[0] 기존 NEFF 정리${NC}"
    rm -rf /tmp/neuron_dep_${NAME}_*

    echo -e "${YELLOW}[1] ${NAME}.py 실행 중...${NC}"
    python3 "${SCRIPT}"
    echo -e "${GREEN}[1] 완료${NC}"

    NEFF_DIRS=$(find /tmp -maxdepth 1 -type d -name "neuron_dep_${NAME}_*" 2>/dev/null | sort)
    if [ -z "$NEFF_DIRS" ]; then
    ...
    for COMPILER_DIR in $NEFF_DIRS; do
        ...
        SHAPE=$(echo "$DIR_NAME" | sed "s/neuron_dep_//")
