#!/bin/bash
# Single Independent Kernel Profiling (bf16)
...
for SCRIPT in "${SCRIPT_DIR}"/*.py; do
    ...
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo -e "${BLUE}  ${NAME} (single independent)${NC}"
    echo -e "${BLUE}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"

    echo -e "${YELLOW}[0] 기존 NEFF 정리${NC}"
    rm -rf /tmp/neuron_single_${NAME}_*

    echo -e "${YELLOW}[1] ${NAME}.py 실행 중...${NC}"
    python3 "${SCRIPT}"
    echo -e "${GREEN}[1] 완료${NC}"

    NEFF_DIRS=$(find /tmp -maxdepth 1 -type d -name "neuron_single_${NAME}_*" 2>/dev/null | sort)
    if [ -z "$NEFF_DIRS" ]; then
    ...
    for COMPILER_DIR in $NEFF_DIRS; do
        ...
        SHAPE=$(echo "$DIR_NAME" | sed "s/neuron_single_${NAME}_//")
        ...
        NTFF_FILE="${OUTPUT_DIR}/profile_${NAME}_${SHAPE}.ntff"
        JSON_FILE="${OUTPUT_DIR}/profile_${NAME}_${SHAPE}.json"
