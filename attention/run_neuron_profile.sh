#!/bin/bash
# LLaMA Attention Neuron Profiling
...
for SCRIPT in "${SCRIPT_DIR}"/*.py; do
    ...
    rm -rf /tmp/neuron_attention_*

    echo -e "${YELLOW}[1] ${NAME}.py 실행 중...${NC}"
    python3 "${SCRIPT}"
    echo -e "${GREEN}[1] 완료${NC}"

    NEFF_DIRS=$(find /tmp -maxdepth 1 -type d -name "neuron_attention_*" 2>/dev/null | sort)
    if [ -z "$NEFF_DIRS" ]; then
    ...
    for COMPILER_DIR in $NEFF_DIRS; do
        ...
        LABEL=$(echo "$DIR_NAME" | sed "s/neuron_//")
        ...
        NTFF_FILE="${OUTPUT_DIR}/profile_${LABEL}.ntff"
        JSON_FILE="${OUTPUT_DIR}/profile_${LABEL}.json"
